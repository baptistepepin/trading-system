...
def add_mov_average(data, column_name, window):
    if column_name not in data.columns:
        return
    data[column_name + " Moving Average " + str(window)] = (
        data[column_name].rolling(window).mean()
    )
...
def add_mov_std(data, column_name, window):
    if column_name not in data.columns:
        return
    data[column_name + " Moving Std " + str(window)] = (
        data[column_name].rolling(window).std()
    )


def add_mov_dynamic(data: pd.DataFrame, column_name: str, window: int):
    if column_name not in data.columns:
        return
    m = data[column_name] - data[column_name].rolling(window).mean()
    dynamic_col = (m) / data[column_name].rolling(window).std()
